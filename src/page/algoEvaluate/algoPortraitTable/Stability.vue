    ...
    <div>
        ...
        <div class="table-container">
            <el-table :data="tableData" size="medium " :row-style="{ height: '56px' }" height="695px">
                ...
                <el-table-column prop="" label="VWAP滑点值标准差">
                    <template slot-scope="scope"> {{ scope.row.vwap_std_dev }}% </template>
                </el-table-column>
                <el-table-column prop="" label="收益率标准差">
                    <template slot-scope="scope"> {{ scope.row.profit_rate_std }}% </template>
                </el-table-column>
                <el-table-column prop="joint_rate" label="贴合度">
                    <template slot-scope="scope"> {{ scope.row.joint_rate }}% </template>
                </el-table-column>
...
export default {
    ...
    methods: {
        getTableData(pageObj = { page: 1, pageNum: 2 }) {
            ...
            let query = { profile_type: 5, start_time: 1658194200, end_time: 1658244600, page: pageObj.page, limit: pageObj.pageNum };
            console.log(query);
            fiveDimensionsApi(query).then((res) => {
                if (res.code == 200) {
                    this.tableData = res.stability;
